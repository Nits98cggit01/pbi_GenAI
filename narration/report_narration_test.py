...
def extract_final_answer(file_path):
    print(f'Triggered this function')
    try:
...
# ROOT_PATH = st.text_input('Enter the path where the data extraction resides')
...
with col1:
    report_type = st.selectbox(
        'Select Report Type', ('Select', 'PowerBI', 'Tableau', 'Cognos', 'Qliksense'))
...
if extraction_counter == True:
    ...
    try:
        if extraction_counter == True:
            ...
            selected_page = st.sidebar.selectbox('Page name', pages, index=0)
            get_details(selected_page)

            metrics = ['Choose the visual', 'All Metrics'] + extract_visuals()
            selected_metrics = st.sidebar.selectbox('Visuals', metrics, index=0)

            section = ['Choose an option','Full report','General analysis']
            selected_section = st.sidebar.selectbox('Report Section', section, index=0)
            ...
            if tabs == 'Details':
                ...
                if selected_page == 'All Pages' or selected_metrics == 'All Metrics':
                    ...
                    qa_result_file = os.path.join(ROOT_PATH,"csv_agent_response.txt")
                    if os.path.isfile(qa_result_file):
                        os.remove(os.path.join(ROOT_PATH,"csv_agent_response.txt"))
                    else:
                        print(f'The qa response file does not exists')
